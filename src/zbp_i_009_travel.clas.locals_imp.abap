CLASS lhc_Travel DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
    ...
    TYPES tt_travel_update TYPE TABLE FOR UPDATE zi_009_travel.
...
CLASS lhc_Travel IMPLEMENTATION.
  ...
  METHOD validate_agency.
    READ ENTITY zi_009_travel\\travel FROM VALUE #(
  ...
  METHOD get_features.
    ...
    READ ENTITY zi_009_travel FROM VALUE #( FOR keyval IN keys
                                                      (  %key                    = keyval-%key
...
**                                                        %control-overall_status = if_abap_behv=>mk-on
    ...
    result = VALUE #( FOR ls_travel IN lt_travel_result
                      ( %key                           = ls_travel-%key
                        %features-%action-acceptTravel = COND #( WHEN ls_travel-travel_id = '100'
  ...
  MODIFY ENTITIES OF zi_009_travel IN LOCAL MODE
         ENTITY travel
            UPDATE FROM VALUE #( FOR key IN keys ( mykey = key-mykey
                                                   travel_id = '100' " Accepted
                                                   %control-travel_id = if_abap_behv=>mk-on ) )
         FAILED   failed
         REPORTED reported.

  " Read changed data for action result
  READ ENTITIES OF zi_009_travel IN LOCAL MODE
       ENTITY travel
...
METHOD calculatetravelkey.
  ...
  "Get max travelID
  SELECT SINGLE FROM z009_travel FIELDS MAX( travel_id ) INTO @DATA(lv_max_travelid).
